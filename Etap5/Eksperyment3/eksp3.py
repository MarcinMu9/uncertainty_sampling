...
# iris = load_iris()
sam = 400  # liczba próbek
B = 70  # budżet (możliwa liczba próbek do sklasyfikowania)
X, y = make_classification(n_samples=sam, n_features=40, n_informative=10, random_state=1)
# X = iris.data
# y = iris.target
...
def uncertainty_sampling(X_pool, y_pool, model, clf, budget):
    ...
    for i in range(budget):
        ...
        np.savetxt('bac8.csv', bac, header='balanced accuracy score (B=70)')
        np.save('bac8.npy', bac)
        us_[i] = us.mean()
        np.savetxt('us8.csv', us_, header='uncertainty score (B=70)')
        np.save('us8.npy', us)
