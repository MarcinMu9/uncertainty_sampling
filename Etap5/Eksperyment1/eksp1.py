...
B = 1  # budżet (możliwa liczba próbek do sklasyfikowania)
...
def uncertainty_sampling(X_pool, y_pool, model, clf, budget):
    ...
    for i in range(budget):
        ...
        np.savetxt('bac3.csv', bac, header='balanced accuracy score (tree)')
        np.save('bac3.npy', bac)
        us_[i] = us.mean()
        # np.savetxt('us.csv', us_, header='uncertainty score')
        # np.save('us.npy', us)
